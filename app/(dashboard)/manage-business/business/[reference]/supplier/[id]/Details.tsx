...
const Details = ({ reference, id }: { reference: string; id: string }) => {
  ...
  const { data, isFetching } = useCustomQuery<BusinessResponse['suppliers']>({
    api: `v1/super-admin/business/${reference}/suppliers/${id}`,
  ...
  const items = [
    ...
    { title: 'Primary email', value: data?.primary_email },
  ];
